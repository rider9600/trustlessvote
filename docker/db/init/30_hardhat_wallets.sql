-- Hardhat wallet assignments table
-- Maps each profile to one of the 20 pre-funded Hardhat accounts

create table if not exists public.hardhat_wallet_pool (
  ...
  constraint hardhat_wallet_pool_profile_fkey foreign key (assigned_to_profile_id) references public.profiles(id) on delete set null
);

-- Insert the 20 Hardhat accounts (public test accounts)
insert into public.hardhat_wallet_pool (wallet_address, private_key) values
...
-- Grant permissions
grant select, insert, update, delete on public.hardhat_wallet_pool to anon, authenticated, service_role;
grant usage, select on sequence hardhat_wallet_pool_id_seq to anon, authenticated, service_role;
alter table public.hardhat_wallet_pool disable row level security;
