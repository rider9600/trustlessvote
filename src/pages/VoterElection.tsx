...
const phaseLabel: Record<
  "registration" | "commit" | "reveal" | "results",
  string
> = {
...
function deriveTimelineAndPhase(election: ElectionWithPhases | null): {
  ...
  if (commitPhase) {
    ...
    if (
      now >= commitStart &&
      (!revealPhase || now <= new Date(commitPhase.end_time))
    ) {
      return {
        status: "ongoing",
        currentPhase: "commit",
  ...
  const startTimes = election.phases.map((p) =>
...
export default function VoterElection() {
  ...
  const loadData = async () => {
    try {
      ...
      setIsLoading(true);
      console.log("Loading election data for:", electionId);
      ...
      toast.error(`Failed to load election details: ${error.message}`);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const {
    ...
    phaseStart,
    phaseEnd,
  } = useMemo(() => deriveTimelineAndPhase(election), [election]);
  ...
  const handleReveal = async () => {
    ...
    try {
      ...
      // Get stored commitment for convenience
      const commitKey = `tvote_commit_${electionId}_${voterStatus.voter_id}`;
      ...
      if (stored) {
        ...
        // Warn if secret doesn't match (but still let them try)
        if (parsed.secretKey !== secretKey) {
          toast.warning(
            "Secret key differs from your stored record. Make sure it's correct."
          );
        }
      }

      if (!candidateId) {
        toast.error("Please select the candidate you voted for");
        return;
      }
    ...
    return (
      <Layout currentPhase="registration" isAdmin={false}>
        <div className="max-w-3xl mx-auto py-16 text-center space-y-4">
          ...
          <Button onClick={() => navigate("/voter")} variant="outline">
  ...
  // Determine if user can commit or reveal
  const canCommit =
      ...
      <div className="max-w-4xl mx-auto space-y-8">
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground max-w-xl">
                View election details, candidates and participate according to
                the current phase.
              </p>
            </div>
          </div>
        ...
        >
          ...
          <p className="text-muted-foreground max-w-2xl mx-auto">
            Select your preferred candidate below. Your vote will be encrypted
            and stored on the blockchain until the reveal phase.
          </p>
        </section>
              ...
              <div className="space-y-1 text-sm">
                ...
                {timelineStatus === "upcoming" && (
                  <p className="text-muted-foreground">
                    This election has not started yet. Once the commit phase
                    opens, you&apos;ll be able to cast your vote.
                  </p>
                )}
                {timelineStatus === "ongoing" && currentPhase === "commit" && (
                  <p className="text-muted-foreground">
                    The commit phase is active. Select a candidate and commit
                    your encrypted vote using a secret key.
                  </p>
                )}
                {timelineStatus === "ongoing" && currentPhase === "reveal" && (
                  <p className="text-muted-foreground">
                    Reveal phase is active. Provide the same secret key you used
                    during commit so your vote can be counted.
                  </p>
                )}
                {timelineStatus === "past" && (
                  <p className="text-muted-foreground">
                    This election has concluded. Your commit and reveal status
          ...
          <div className="official-card p-6 space-y-4">
            ...
            {canCommit && (
              <div className="space-y-4">
                <div className="flex items-center gap-3 text-sm text-muted-foreground">
                  ...
                  <p>
                    Select a candidate above, then choose a secret key. You will
                    need the same key again during the reveal phase.
                  </p>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    This key never leaves your device except as part of the
            ...
            {canReveal && (
              <div className="space-y-4">
                <div className="flex items-center gap-3 text-sm text-muted-foreground">
                  <AlertCircle className="w-4 h-4" />
                  <p>
                    Enter the same secret key you used when committing your vote
                    so it can be decrypted and counted.
                  </p>
                </div>
            ...
            {!canCommit && !canReveal && (
              <div className="flex items-center gap-3 text-sm text-muted-foreground">
                ...
                <p>
                  There is no commit or reveal action available for you at this
                  time. Check the timeline above for phase details.
                </p>
              </div>
            )}
          </div>
        ...
        >
          <div className="text-center">
            ...
            <p className="text-muted-foreground mt-2">
              A secure commit-reveal voting mechanism ensures privacy and
              integrity.
            </p>
          </div>
          ...
          <div className="grid md:grid-cols-4 gap-6">
            {featureCards.map((feature) => (
              <div
                key={feature.title}
                className="official-card p-5 text-center hover-lift"
              >
